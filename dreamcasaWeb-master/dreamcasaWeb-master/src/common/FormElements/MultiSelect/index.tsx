...
export interface IMultiSelectProps {
  ...
  openButtonCls?: string //used for styling of select button when it is opened
  id?: string
  optionsInterface: { isObj: boolean, displayKey?: string } //used when options are complex objects like option is {id:1, label:"open lands"} then give optionsInterface as {isObj:true, displaykey:"label"} else if option is string or number like "open lands" then give optionsInterface as {isObj:false} no need of display keys this is used that any type of options can be just need display key to show which field should we need to show in the ui
  options: Array<any>
                ...
                >
                  {({ selected }) => (
                    <div className={twMerge(
                      clsx({
                        "px-3 py-2 mb-1 hover:bg-slate-500 hover:text-slate-900 cursor-pointer": true,
