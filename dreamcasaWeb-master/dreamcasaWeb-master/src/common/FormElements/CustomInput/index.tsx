...
export interface ICustomInputOtherProps {
  ...
  maxLength?: number;
  noteCls?: string;
  type: 'text' | 'textarea' | 'number' | 'url' | 'email' | 'password' | 'tel';
  outerInptCls?: string;
  leftIcon?: ReactNode;
  placeholder?: string;
  rightIcon?: ReactNode;
        ...
        {type === 'textarea' ? (
          <div className="relative">
            <RenderInput
              ...
              className={twMerge(
                clsx({
                  'w-full px-4 py-3 bg-white border-2 label-text rounded-xl text-[13px] text-slate-800 transition-all duration-200 ease-out':
                    true,
          ...
          >
            ...
            <RenderInput
              ...
              className={twMerge(
                clsx({
                  'flex-1 min-w-0 bg-transparent text-[13px] label-text text-slate-800 focus:outline-none':
                    true,
                  'placeholder:text-slate-400 placeholder:text-[13px]': true,
            ...
            {unitsDropdown && (
              <select
                ...
                className="text-[11px] md:text-[12px] bg-[#3586FF] hover:bg-[#2d75e6] rounded-md text-white px-2 md:px-3 py-[2px] md:py-0.5 font-medium border-0 focus:outline-none cursor-pointer transition-colors duration-200 flex-shrink-0 min-w-[70px] appearance-none text-center"
                style={{ backgroundImage: 'none' }}
              >
                <option value="" className="bg-white text-slate-700 text-[12px]">
                  Units
                </option>
                {unitsDropdown.options.map((option) => (
                  <option key={option} value={option} className="bg-white text-slate-700 text-[12px]">
                    {option}
                  </option>
                ))}
              </select>
