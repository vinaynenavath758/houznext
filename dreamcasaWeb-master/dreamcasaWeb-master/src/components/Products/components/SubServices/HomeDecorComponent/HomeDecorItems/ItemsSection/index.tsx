...
import Item from "../../../FurnitureComponent/furnitureItems/ItemsSection/item";
...
export default function ItemsSection({
  ...
  return (
    <>
      ...
      <Drawer open={mobileSortOpen} handleDrawerToggle={setMobileSortOpen} title="SORT BY" hideHeader={false} openVariant="right" closeOnOutsideClick panelCls="w-full max-w-[100%] sm:max-w-[340px]" panelInnerCls="overflow-hidden px-0 py-0 flex flex-col" closeIconCls="text-gray-700" overLayCls="bg-black/40" className="!z-[200]">
        <div className="flex flex-col flex-1 min-h-0">
          ...
          <div className="shrink-0 flex items-center gap-2 border-t border-gray-200 bg-white px-3 py-2">
            <button type="button" onClick={() => setMobileSortOpen(false)} className="flex-1 py-2.5 text-sm font-semibold text-gray-700 uppercase tracking-wide rounded border border-gray-300 bg-white">
              Close
            </button>
            <button type="button" onClick={() => setMobileSortOpen(false)} className="flex-1 py-2.5 text-sm font-semibold text-white bg-[#3586FF] rounded uppercase tracking-wide">
              Apply
            </button>
          </div>
        </div>
      ...
      <Drawer open={mobileFilterOpen} handleDrawerToggle={setMobileFilterOpen} title="FILTERS" hideHeader={false} openVariant="right" closeOnOutsideClick panelCls="w-full max-w-[100%] sm:max-w-[360px]" panelInnerCls="overflow-hidden px-0 py-0 flex flex-col" closeIconCls="text-gray-700" overLayCls="bg-black/40" className="!z-[200]">
        <div className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 min-h-0 overflow-y-auto">
            <ItemsFilterBar
              items={items as any}
          ...
          <div className="shrink-0 flex items-center gap-2 border-t border-gray-200 bg-white px-3 py-2">
            <button type="button" onClick={() => setMobileFilterOpen(false)} className="flex-1 py-2.5 text-sm font-semibold text-gray-700 uppercase tracking-wide rounded border border-gray-300 bg-white">
              Close
            </button>
            <button type="button" onClick={() => setMobileFilterOpen(false)} className="flex-1 py-2.5 text-sm font-semibold text-white bg-[#3586FF] rounded uppercase tracking-wide">
              Apply
            </button>
          </div>
        </div>
